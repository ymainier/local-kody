...
import { describeInput, getCapability, listCapabilities, type Capability } from './registry.ts'
...
export const searchInputSchema = z.object({
	...
	entity: z
		...
		.optional()
		.describe('Open refs such as "capability:notifySelf", "package:@me/x", "guide:packages"'),
})
...
const stopwords = new Set(['a', 'an', 'the', 'to', 'me', 'my', 'i', 'for', 'of', 'and', 'or', 'on', 'in', 'it', 'with', 'is', 'when', 'what', 'how', 'do', 'can', 'you'])
...
function readGuides() {
	return readdirSync(guidesDir)
		...
		.map((file) => {
			...
			return { id: file.replace(/\.md$/, ''), title, summary: summary.trim(), markdown }
		})
}

function callShape(capability: Capability) {
	const keys = Object.keys((z.toJSONSchema(capability.inputSchema) as { properties?: object }).properties ?? {})
...
function buildEntries(): Array<SearchEntry> {
	...
	return [...capabilityEntries, ...packageEntries, ...guideEntries, ...secretEntries]
}
...
function packageDetail(manifest: PackageManifest) {
	const exportLines = Object.entries(manifest.exports).map(([exportName, file]) => {
		const leaf = exportName.slice(2)
		return `- \`${exportName}\` (${file}): \`import ${leaf.replace(/[^a-zA-Z0-9]/g, '')} from 'kody:${manifest.name}/${leaf}'\``
	})
	return [`## package:${manifest.name}`, manifest.description, '', 'Exports:', ...exportLines].join('\n')
}
...
function domainIndex(entries: Array<SearchEntry>) {
	...
	for (const entry of entries) {
		domains.set(entry.domain, [...(domains.get(entry.domain) ?? []), entry.title])
	}
	const rows = [...domains].map(
		([domain, titles]) => `- **${domain}** (${titles.length}): ${titles.slice(0, 4).join(', ')}`,
	)
	return ['# Domains', ...rows, '', 'Follow up with { "domain": "<id>" }, { "query": "..." } or { "entity": "<type>:<id>" }.'].join('\n')
}
...
export function search(input: SearchInput) {
	...
	const entries = buildEntries().filter((entry) => !input.domain || entry.domain === input.domain)
	if (!input.query) {
		if (!input.domain) return domainIndex(entries)
		return [`# Domain ${input.domain}`, ...entries.map((entry) => `- \`${entry.ref}\`: ${entry.summary}`)].join('\n')
	}
	...
	const lines = ranked.map(({ entry }, index) => {
		const shape = entry.callShape && index < 3 ? ` Call: \`${entry.callShape}\`` : ''
		return `- \`${entry.ref}\`: ${entry.summary}${shape}`
	})
	return ['# Search results', ...lines, '', 'Open one with { "entity": "<ref>" } for types and a ready-to-run module.'].join('\n')
}
