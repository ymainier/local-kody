...
defineCapability({
	...
	keywords: ['notify', 'notification', 'alert', 'ping', 'remind', 'tell me', 'message'],
...
defineCapability({
	...
	inputSchema: z.object({
		...
		exports: z.record(z.string(), z.string()).describe('"./name" -> "./file.ts"'),
	}),
	async handler(input) {
...
const storageInput = z.object({
	namespace: z.string().regex(/^[a-z0-9-]+$/).describe('Usually the package leaf'),
	key: z.string().min(1),
})
...
defineCapability({
	name: 'storageGet',
	domain: 'storage',
	description: 'Read a JSON value saved with storageSet. Returns null when missing.',
...
defineCapability({
	name: 'storageSet',
	domain: 'storage',
	description: 'Save a JSON value (a cursor, last-seen id, settings) that survives between runs.',
	...
	async handler({ namespace, key, value }) {
		...
		data[key] = value
		writeFileSync(join(storageDir, `${namespace}.json`), JSON.stringify(data, null, 2))
		return { saved: true }
	},
})
