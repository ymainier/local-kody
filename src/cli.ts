...
function readHosts(args: Array<string>) {
	...
	for (let index = 0; index < args.length; index += 1) {
		if (args[index] === '--host' && args[index + 1]) hosts.push(args[index + 1] ?? '')
	}
	return hosts
}
...
switch (command) {
	case 'set': {
		...
		if (!name || !value) throw new Error('Usage: set <name> <value> --host <host>')
		setSecret(name, value, readHosts(rest))
		console.log(`Saved secret "${name}" for ${readHosts(rest).join(', ') || 'no hosts yet'}`)
		break
	}
	case 'allow': {
	...
	case 'list':
		...
		break
	default:
		console.log('Commands: set <name> <value> --host <host> | allow <name> <host> | list')
}
