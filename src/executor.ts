...
async function buildImportMap(code: string, runtimePath: string) {
	...
	while (queue.length > 0) {
		...
		for (const { specifier } of found) {
			...
			if (/^(\.|\/|kody:|npm:|jsr:|node:|https?:|data:)/.test(specifier)) continue
...
function sandboxEnv() {
	const passThrough = ['HOME', 'PATH', 'DENO_DIR', 'DENO_CERT', 'HTTPS_PROXY', 'HTTP_PROXY']
...
export async function execute(input: {
	...
	const unregister = registerRun(runId, { logs, settle: (outcome) => (settled = outcome) })
	const runDir = await mkdtemp(join(tmpdir(), 'kody-run-'))
	const finish = (outcome: { result?: unknown; error?: string }): ExecuteOutcome => {
		const durationMs = Math.round(performance.now() - startedAt)
		const size = JSON.stringify(outcome.result ?? null).length
		if (size > maxResultBytes) {
			return { error: `Result is ${size} bytes; return less than ${maxResultBytes}.`, logs, durationMs }
		}
	...
	try {
		...
		let stderr = ''
		child.stdout.on('data', (chunk: Buffer) => logs.push(chunk.toString('utf8').trimEnd()))
		...
		clearTimeout(timer)
		if (timedOut) return finish({ error: `Execution timed out after ${timeoutMs} ms` })
		if (settled) return finish(settled)
		return finish({ error: stderr.trim() || 'Sandbox exited without a result' })
	} catch (error) {
