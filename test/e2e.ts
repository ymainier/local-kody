...
type ExecuteResult = { result?: unknown; error?: string; logs: Array<string>; durationMs: number }
...
const api = createServer((request, response) => {
	...
	response.end(
		JSON.stringify([
			{ id: '3', type: 'ReleaseEvent', repo: { name: 'kody-bot/tool' }, payload: { action: 'published' } },
			{ id: '2', type: 'PushEvent', repo: { name: 'kody-bot/tool' }, payload: {} },
			{ id: '1', type: 'CreateEvent', repo: { name: 'kody-bot/new' }, payload: { ref_type: 'repository' } },
		]),
	)
})
...
writeFileSync(
	...
	JSON.stringify({ githubToken: { value: 'gh-test-token', allowedHosts: ['127.0.0.1'] } }),
)
...
async function callText(name: string, args: Record<string, unknown>) {
	const response = (await client.callTool({ name, arguments: args })) as ToolText
	return { text: response.content[0]?.text ?? '', isError: response.isError === true }
}
...
async function step(name: string, check: () => Promise<string>) {
	try {
		...
		report.push({ step: name, ok: false, detail: (error as Error).message.slice(0, 160) })
	}
}
...
await step('"ping me when it is done" ranks notifySelf first', async () => {
	const { text } = await callText('search', { query: 'ping me when it is done' })
	assert.match(text.split('\n')[1] ?? '', /capability:notifySelf/)
	return text.split('\n')[1] ?? ''
})

await step('entity lookup returns a ready-to-run module', async () => {
	const { text } = await callText('search', { entity: 'capability:notifySelf' })
	assert.match(text, /kody\.notifySelf\(params\)/)
	return 'snippet present'
})

await step('npm import + capability + secret placeholder in one module', async () => {
...
export default async function main(params) {
  ...
  return { status: response.status, events: events.length, secrets: secrets.map((s) => s.name), age: formatDistanceStrict(new Date(2026, 0, 1), new Date(2026, 8, 19)) }
	...
	assert.deepEqual(outcome.result, { status: 200, events: 3, secrets: ['githubToken'], age: '9 months' })
...
await step('search for the task now finds the saved package first', async () => {
	const { text } = await callText('search', { query: 'what did kody-bot ship' })
...
export default async function main(params) { return await whatShipped(params) }`
	...
	assert.deepEqual((first.result as { shipped: Array<string> }).shipped, ['ReleaseEvent kody-bot/tool', 'CreateEvent kody-bot/new'])
...
await step('secret refused for a host it is not approved for', async () => {
	const outcome = await run(`export default async function main(params) {
...
await step('sandbox cannot read files, env, spawn or open sockets', async () => {
  ...
  return {
	...
	assert.deepEqual(outcome.result, { readFile: 'NotCapable', env: 'NotCapable', spawn: 'NotCapable', socket: 'NotCapable' })
